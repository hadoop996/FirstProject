    ...
    public static <R> R doGet(final String url,
        ...
        try (final CloseableHttpClient httpClient = HttpClients.createDefault()) {
            ...
            if (!CollectionUtils.isEmpty(headerMap)) {
                headerMap.entrySet().stream().map(HttpUtils::convertEntryToBasicHeader).forEach(httpReq::setHeader);
            }
...
//        sc.init(null, new TrustManager[]{trustManager}, new java.security.SecureRandom());
    ...
    private static List<NameValuePair> convertMapToBasicNameValuePairs(final Map<String, String> paramsMap) {
        ...
        return paramsMap.entrySet().stream()
                .map(HttpUtils::convertEntryToBasicNameValuePair)
...
//        body.put("wd", "jmeter性能测试");
        HttpUtils.doGet(baidu, (status, res) -> {
            System.out.println(status);
            System.out.println(res);
            return res;
        }, body);
